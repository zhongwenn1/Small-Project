...
from runningTime import TimeMeasure
...
class DataProc:
	...
	def linkedBar(self, df):
		self.linked[['Beam Diff']].plot(kind='hist',bins=100,rwidth=0.8)
		plt.title("Linked Bags")
		plt.savefig('link.png')

	def unlinkedBar(self, df):
		self.unlinked[['Beam Diff']].plot(kind='hist',bins=100,rwidth=0.8)
		plt.title("Unlinked Bags")
