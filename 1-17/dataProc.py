...
class DataProc:
  ...
  def processLines(self):
    ...
    self.linkedreason(self.dfinal)

  def linkedreason(self, df):
    plt.subplots(figsize=(24,8))
    self.linked['Beam Diff'].value_counts().plot(kind='bar')
    plt.xticks(rotation=45, fontsize=16)
    plt.yticks(fontsize=20)
    plt.show()

  # need to modify
  def timeVSunlinked(self, df):
    # time_unlinked = df['Time'].value_counts().index
    # time_ratio = df['Time'].value_counts().values
  ...
  def getDiffHistogram(self, df):
    ...
    plt.figtext(.49,.66,'Rate for slipping bags (151 to 200): {0:.2f} % \nSlipping bag rate out of unlinked: {1:.2f} %'.format(rate_range151to200_total, rate_range151to200_unlinked),fontsize=9,ha='left')
    plt.figtext(.49,.60,'Rate for pushed bags (-1 to -50): {0:.2f} % \nPushed bag rate out of unlinked: {1:.2f} %'.format(rate_rangem1tom50_total, rate_rangem1tom50_unlinked),fontsize=9,ha='left')
